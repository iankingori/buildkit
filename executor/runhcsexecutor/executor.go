...
var defaultCommandCandidates = []string{"runhcs"}

type runhcsExecutor struct {
	runc             *runc.Runc
	root             string
	cgroupParent     string
	rootless         bool
...
func New(opt Opt, networkProviders map[pb.NetMode]network.Provider) (executor.Executor, error) {
	...
	runtime := &runc.Runc{
		Command:   cmd,
		Log:       filepath.Join(root, "runhcs-log.json"),
		LogFormat: runc.JSON,
		Setpgid:   true,
...
func (w *runhcsExecutor) Run(ctx context.Context, id string, root executor.Mount, mounts []executor.Mount, process executor.ProcessInfo, started chan<- struct{}) (rec resourcestypes.Recorder, err error) {
	...
	defer mount.UnmountAll(bundle, 0)
	...
	// if err := fs.CopyFile(bundle+"\\sandbox.vhdx", rootMount[0].Source+"\\sandbox.vhdx"); err != nil {
	// 	return nil, err
	// }

	parentLayerPaths, err := rootMount[0].GetParentPaths()
	...
	if err != nil {
		...
		ids, err := windows.ResolveUsernameToSID(ctx, w, rootMount, meta.User)
		if err != nil {
			return nil, err
		}
	...
	if runtime.GOOS == "windows" {
		opts = append(opts, containerdoci.WithWindowsLayerFolders(parentLayerPaths[len(parentLayerPaths)-1], rootMount[0].Source))
	}

	defer mount.Unmount(rootMount[0].Source, 0)
	...
	releaseContainer := func(ctx context.Context) error {
		err := w.runc.Delete(ctx, id, &runc.DeleteOpts{Force: true})
...
func (w *runhcsExecutor) Exec(ctx context.Context, id string, process executor.ProcessInfo) (err error) {
